...
for freq in np.linspace(cfg.fr - bw, cfg.fr + bw, 40): #goes through frequencies (40) around bandwidth of resonant frequency
    C_treated, C_untreated = simul(freq, 600)
    Ct.append([freq, C_treated]) 
    kbac = (1-C_treated/C_untreated)*100
    kb.append([freq, kbac])



# The following lines of code until plt.show() were generated using AIx``

freqs = [x[0] for x in kb]
kbacs = [x[1] for x in kb]

plt.plot(freqs, kbacs)
plt.xlabel("Frequency (Hz)")
plt.ylabel("Bacteria Killed (%)")
plt.title("Percentage of Bacteria Killed vs Frequency around Resonance")
plt.show()
